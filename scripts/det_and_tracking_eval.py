...
# Buffer for all classes (players=0, gk=1, ball=2, ref=3)
@dataclass
class MotRow:
    frame: int; cls: int; tid: int
    x: float; y: float; w: float; h: float
    conf: float  # per-frame box confidence; use 1.0 if unknown

mot_rows = []  # will fill during the loop

def open_mot_writer(video_path: str, out_dir: str):
    os.makedirs(out_dir, exist_ok=True)
    seq = Path(video_path).stem
    out_path = os.environ.get("SN_RESULTS_TXT", os.path.join(out_dir, f"{seq}.txt"))
    return open(out_path, "w"), out_path

# === YOLO detection label helpers ===
def open_yolo_det_dir(video_path: str, root: str):
    """
    Returns the directory where per-frame YOLO TXT labels will be written.
    Layout: <root>/<seq_name>/labels/000001.txt, ...
    """
    seq = Path(video_path).stem
    out_dir = os.path.join(root, seq, "labels")
    os.makedirs(out_dir, exist_ok=True)
    return out_dir

def write_yolo_labels_for_frame(label_dir: str, frame_idx: int, dets_np: np.ndarray,
                                img_w: int, img_h: int, keep_classes=None):
    """
    dets_np rows: [x1,y1,x2,y2,conf,cls]
    Writes YOLO TXT (class x_c y_c w h) normalized to [0,1].
    If no detections, writes an empty file (useful for some evaluators).
    """
    p = os.path.join(label_dir, f"{frame_idx:06d}.txt")
    with open(p, "w") as f:
        if dets_np is None or dets_np.size == 0:
            return
        for x1,y1,x2,y2,conf,cls in dets_np:
            cls = int(cls)
            ...
            y_c = y1 + 0.5 * h
            # normalize & clip
            xn = min(max(x_c / img_w, 0.0), 1.0)
            yn = min(max(y_c / img_h, 0.0), 1.0)
            wn = min(max(w / img_w,   0.0), 1.0)
            hn = min(max(h / img_h,   0.0), 1.0)
            f.write(f"{cls} {xn:.6f} {yn:.6f} {wn:.6f} {hn:.6f}\n")
...
seq = Path(video_path).stem
output_video = os.path.join(output_dir, f"{seq}_output_video.mp4")
video_writer = cv2.VideoWriter(output_video, fourcc, fps, (width, height))

# Data-Writer
mot_file_path = os.path.join(output_dir, "021_mot_results.txt")
output_rows = []
...
colors = {
    2:(0,0,255),   # Ball
    3:(255,255,0), # Ref
    4:(255,128,0), # Side Ref (neu)
    5:(128,128,128) # Others (neu)
}
# Wichtig: Dein YOLO muss diese Klassen auch wirklich ausgeben.
target_classes = [0,1,2,3,4,5]
class_names = {0:"player", 1:"keeper", 2:"ball", 3:"ref", 4:"side-ref", 5:"other"}
...
for class_id in target_classes:
    ...
    elif class_id in [1,3,4,5]: # Ref oder Keeper
        trackers_dict[class_id] = BotSort(
            with_reid=False,
            reid_weights=None,
            device=mot_device,
...
mot_out_dir = "output/challenge/soccernet_mot"
mot_writer, mot_path = open_mot_writer(video_path, mot_out_dir)

# after cap is opened & width/height known
yolo_det_root = "outputDet"                 # top-level for detection labels
yolo_det_dir  = open_yolo_det_dir(video_path, yolo_det_root)
...
while True:
    ...
    dets = np.array(dets)
    write_yolo_labels_for_frame(
        yolo_det_dir,
        frame_idx,
        dets,                # shape Nx6 [x1,y1,x2,y2,conf,cls]
        width, height,
        keep_classes=target_classes   # or None to keep all classes
    )
    ...
    # Nicht-Player zeichnen: Ball(2), Ref(3), SideRef(4), Others(5)
    for cid in [2, 3, 4, 5]:
        for t in results_by_class[cid]:
            x1, y1, x2, y2, local_tid = map(int, t[:5])
            w, h = x2 - x1, y2 - y1

            # Challenge-Buffer
            mot_rows.append(MotRow(frame_idx, cid, local_tid, float(x1), float(y1), float(w), float(h), 1.0))

            # Analytics
            output_rows.append((frame_idx, local_tid, float(x1), float(y1), float(w), float(h), -1.0, str(cid), -1))

            # Preview (optional)
            col = colors.get(cid, (200,200,200))
            cv2.rectangle(frame, (x1, y1), (x2, y2), col, 2)
            lbl = class_names.get(cid, f"C{cid}")
            cv2.putText(frame, f"{lbl} ID{local_tid}", (x1, y1 - 5),
    ...
    for t in results_by_class[1]:
        x1, y1, x2, y2, local_tid = map(int, t[:5])  # echte ID ignorieren
        w, h = x2 - x1, y2 - y1
        ...
        mot_rows.append(MotRow(frame_idx, 1, local_tid, float(x1), float(y1), float(w), float(h), 1.0))
        output_rows.append((frame_idx, local_tid, float(x1), float(y1), float(w), float(h), -1.0, f"GK-{side}", -1))
    ...
    if kits_clf is not None and kit_colors:
        ...
        for lbl,(x1,y1,x2,y2,track_id) in zip(labels, bboxes):
            ...
            mot_rows.append(MotRow(frame_idx, 0, track_id, float(x1), float(y1), float(x2 - x1), float(y2 - y1), 1.0))
...
# === Datei schreiben ===
# --- Build final per-object local ID after merging ---
def final_tid_for(cls_id: int, local_tid: int) -> int:
    # Only players (cls=0) are stitched; others keep their local IDs
    if cls_id == 0:
        return int(tid_new_id.get(local_tid, local_tid))
    return int(local_tid)

# --- Global, class-agnostic ID namespace for submission ---
key2gid = {}
next_gid = 1
for r in mot_rows:
    k = (r.cls, final_tid_for(r.cls, r.tid))
    if k not in key2gid:
        key2gid[k] = next_gid
        next_gid += 1

# --- Now write the CHALLENGE file (10 cols) using FINAL IDs ---
def _final_gid(r):
    return key2gid[(r.cls, final_tid_for(r.cls, r.tid))]

for r in sorted(mot_rows, key=lambda m: (m.frame, _final_gid(m))):
    gid = _final_gid(r)
    mot_writer.write(f"{r.frame},{gid},{r.x:.2f},{r.y:.2f},{r.w:.2f},{r.h:.2f},{r.conf:.2f},-1,-1,-1\n")
